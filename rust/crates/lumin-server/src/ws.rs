...
//! Includes heartbeat ping every `ws_heartbeat_ms` to keep connections alive.
...
async fn handle_ws(mut socket: WebSocket, state: Arc<AppState>) {
    ...
    // Heartbeat interval (mirrors TS 30s ping/pong)
    let heartbeat_interval = Duration::from_millis(state.config.server.ws_heartbeat_ms);
    let mut heartbeat_timer = tokio::time::interval(heartbeat_interval);
    heartbeat_timer.tick().await; // skip the first immediate tick

    // Process incoming messages with heartbeat
    loop {
        tokio::select! {
            // Heartbeat: send ping frame periodically
            ...
            msg = socket.recv() => {
                ...
                let text = match msg {
                    ...
                    Message::Pong(_) => continue, // heartbeat response — client alive
